...
export class SearchStaffComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(this.incomingToggleValues)  {
      this.inactiveStaff=this.incomingToggleValues;
    }else{
  ...
  search() {
    ...
    this.toggelValues.emit(this.inactiveStaff);
    this.filterParams.emit(this.getAllStaff.filterParams)
    ...
    this.studentAttendanceService.staffListForMissingAttendance(this.getAllStaff).subscribe(res => {
      if (res._failure) {
        this.commonService.checkTokenValidOrNot(res._message);
        this.searchList.emit(this.currentForm.value);
        this.snackbar.open(res._message, '', {
          duration: 10000
        });
        this.checkSearchRecord = 0;
