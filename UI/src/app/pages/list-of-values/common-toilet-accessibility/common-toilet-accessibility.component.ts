...
export class CommonToiletAccessibilityComponent implements OnInit {
  ...
  sortLovList(event: CdkDragDrop<string[]>) {
    if (event.currentIndex > event.previousIndex) {
      this.commonToiletAccessibilityList.filteredData[event.currentIndex].sortOrder = Number(this.commonToiletAccessibilityList.filteredData[event.currentIndex].sortOrder) - 1;
    }
    else if (event.currentIndex < event.previousIndex) {
      this.commonToiletAccessibilityList.filteredData[event.currentIndex].sortOrder = Number(this.commonToiletAccessibilityList.filteredData[event.currentIndex].sortOrder) + 1;
    }
    this.commonToiletAccessibilityList.filteredData[event.previousIndex].sortOrder = Number(event.currentIndex) + 1;

    let dropdownListMod = this.commonToiletAccessibilityList.filteredData?.sort((a, b) => a.sortOrder < b.sortOrder ? -1 : 1);
