...
DROP VIEW IF EXISTS [dbo].[student_list_view];  
                    ...
                    INNER JOIN dbo.student_master ON (((dbo.student_enrollment.tenant_id = dbo.student_master.tenant_id)
                        AND (dbo.student_enrollment.school_id = dbo.student_master.school_id)
                        AND (dbo.student_enrollment.student_id = dbo.student_master.student_id))))
                    LEFT OUTER JOIN dbo.sections ON (((dbo.student_master.tenant_id = dbo.sections.tenant_id)
                        AND (dbo.student_master.school_id = dbo.sections.school_id)
                        AND (dbo.student_master.section_id = dbo.sections.section_id)))),
                        (SELECT dbo.student_enrollment.tenant_id,dbo.student_enrollment.student_id,dbo.student_enrollment.school_id, MAX(dbo.student_enrollment.enrollment_date) AS enrollment_date
                                 FROM dbo.student_enrollment
                                 GROUP BY dbo.student_enrollment.tenant_id,dbo.student_enrollment.school_id,dbo.student_enrollment.student_id) max_enrollment_date
                              WHERE dbo.student_enrollment.student_id=max_enrollment_date.student_id
                              AND dbo.student_enrollment.tenant_id = max_enrollment_date.tenant_id
                              AND dbo.student_enrollment.enrollment_date=max_enrollment_date.enrollment_date
                              AND dbo.student_enrollment.school_id=max_enrollment_date.school_id
                    GO";
